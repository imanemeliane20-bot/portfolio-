...
import { ArrowUpRight, FileText, X, ChevronLeft, ChevronRight } from "lucide-react";
...
const projects = [
  ...
  
  {
    ...
    category: "frontend",
    title: "Portfolio",
    description: "My personal developer portfolio built with Next.js, featuring smooth animations, internationalization, and a polished dark/light theme.",
...
function Carousel3D({ items, onSeeAll }: CarouselProps) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const onMouse = (e: MouseEvent) => {
        ...
        gsap.to(carouselRef.current, { rotationX: rx, rotationZ: rz, duration: 1.2, ease: "power2.out" });
      };
              ...
              <div className="relative z-10 h-full flex flex-col px-3 pt-3 ">
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {project.teck.slice(0, 4).map((t) => (
                    <span key={t} className="text-[9px] px-2 py-0.5 rounded-full text-primary border">
                      {t}
                    </span>
                  ))}
...
const Projects = () => {
  ...
  // TO THESE:
const tabProjects = activeTab === "all" ? projects : projects.filter((p) => p.category === activeTab);
const allTabProjects = tabProjects;
  return (
    <motion.section
      id="projects"
      initial={{ opacity: 0 }}
    ...
    >
      <div className="container mx-auto w-full">
        ...
        <div className="mb-10 xl:mb-14">
          <h2 className="h2 flex items-center justify-center text-5xl text-accent font-bold"
          ...
          <div className="mt-3">
           {/* Scroll hint */}
        <span className="text-secondary flex items-center justify-center font-semibold text-xl uppercase"
         style={{ fontFamily: "'Caveat',sans-serif" }}
        >Scroll to rotate</span>
          </div>
        </div>

        {/* Category Tabs */}
        <div className="flex-wrap flex items-center justify-center gap-2">
          {categories.map((cat) => {
           const count = cat === "all" ? projects.length : projects.filter((p) => p.category === cat).length;
            return (
              ...
              >
                ...
                {count > 0 && (
                  <span className={`ml-1.5 text-base text-accent ${activeTab === cat ? "text-white/70" : "text-accent/50"}`}
                   style={{ fontFamily: "'Caveat',sans-serif" }}
                  >
                    {count}
                  </span>
  ...
  <motion.div
    ...
    exit={{ opacity: 0 }}
  >
    <AllProjects items={allTabProjects} onClose={() => setShowAll(false)} />
  </motion.div>
) : (
            <motion.div key={`carousel-${activeTab}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <Carousel3D items={tabProjects} onSeeAll={() => setShowAll(true)} />
            </motion.div>
          )}
        </AnimatePresence>
