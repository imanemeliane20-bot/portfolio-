...
export default function Contact() {
  ...
  const handleEmail = (e: React.FormEvent) => {
    e.preventDefault();
    const subject = encodeURIComponent(`Message from ${form.fullName}`);
    const body = encodeURIComponent(
      `Name: ${form.fullName}\nEmail: ${form.email}\n\n${form.description}`
    );
    window.open(`mailto:hello@yourname.com?subject=${subject}&body=${body}`);
    setSent(true);
  };

  const handleWhatsApp = () => {
    const text = encodeURIComponent(
      `Hi! I'm ${form.fullName} (${form.email}).\n\n${form.description}`
    );
    window.open(`https://wa.me/0670952218?text=${text}`, "_blank");
    setSent(true);
  };
  ...
  return (
    <section id="contact" className="min-h-screen w-full flex items-center bg-ring">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 lg:px-12 py-20 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        <div style={{ fontFamily: "'Caveat',sans-serif" }} >
          ...
          <p className="mt-8 text-primary text-3xl leading-relaxed max-w-xl">
            ...
            to life with elegant code and thoughtful design.
          </p>        
        </div>
          ...
          {sent ? (
            ...
            <div className="flex flex-col items-center justify-center text-center gap-5 py-10">
              ...
              <p className="text-primary text-sm leading-relaxed max-w-xs">
                ...
                <span className="text-accent font-medium">{form.fullName}</span>!
            ...
            /* ── FORM ── */
            <>
              <h3 className="text-primary text-2xl font-semibold mb-1">Send a message</h3>
              <p className="text-primary text-sm mb-7">I'll get back to you within 24 hours.</p>
